  ...
  for (const testCase of TransformTxTestCases) {
    it(testCase.testName, () => {
      ...
      if (testCase.expectedError) {
        expect(() => transformTx(tx, testCase.costModels, testCase.usedCostModelLanguages)).to.throw(
          testCase.expectedError,
        )
      } else {
        const transformedTx = transformTx(tx, testCase.costModels, testCase.usedCostModelLanguages)
