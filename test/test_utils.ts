...
export const ipv4ToBuffer = (ipv4: string): FixlenBuffer<4> =>
    Buffer.from(ipv4.split('.').map((n) => Number.parseInt(n, 10))) as FixlenBuffer<4>

export const toFixlenBuffer = <N extends number>(str: string, length: N): FixlenBuffer<N> =>
    Buffer.from(str, 'hex') as FixlenBuffer<typeof length>

export const toMaxLenString = <N extends number>(str: string, length: N): MaxlenString<N> =>
    str as MaxlenString<typeof length>
...
export const registerTests = <T>(name: string, parseFn: Parser<T>, validTestcases: ValidParseTestcase<T>[], invalidTestcases: InvalidParseTestcase[]) =>
    describe(name, () => {
        describe('Valid', () => {
        ...
        describe('Invalid', () => {
            for (const {testname, cbor, errMsg: expectedErrMsg} of invalidTestcases) {
                it(testname, () => {
                    expect(
                        () => parseFn(decodeCbor(Buffer.from(cbor, 'hex'))),
                    ).to.throw(ParseError, expectedErrMsg)
                })
            }
        })
    })
